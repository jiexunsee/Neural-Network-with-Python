from numpy import exp, array, random, dot, multiply

class NeuralNetwork():
	def __init__(self):
		random.seed(1)

		# 3 layers in the neural network
		# assign random weights to matrices in network. more weights (nodes), more confident in prediction
		# format is (no. of nodes in previous layer) x (no. of nodes in next following layer)
		self.synaptic_weights1 = 2 * random.random((3, 4)) -1
		self.synaptic_weights2 = 2 * random.random((4, 8)) -1
		self.synaptic_weights3 = 2 * random.random((8, 1)) -1
	...
	def forward_pass(self, inputs):
		...
		a2 = self.__sigmoid(dot(inputs, self.synaptic_weights1)) #4x5
		a3 = self.__sigmoid(dot(a2, self.synaptic_weights2)) #4x4
		output = self.__sigmoid(dot(a3, self.synaptic_weights3)) #4x1
		return output
